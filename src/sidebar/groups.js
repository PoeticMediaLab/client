...
var stateUtil = require('./util/state-util');
...
function groups(annotationUI, localStorage, serviceUrl, session, $rootScope, store) {
  ...
  function getDocumentUriForGroupSearch() {
    ...
    return stateUtil.awaitStateChange(annotationUI, mainUri);
  }
  ...
  function load() {
    return getDocumentUriForGroupSearch().then(uri => {
      ...
      if (focusedGroupId) {
        var focusedGroupInFetchedList = gs.find(g => g.id === focusedGroupId);
        if (!focusedGroupInFetchedList) {
          focusedGroupId = gs[0].id;
          focus(gs[0].id);
          $rootScope.$apply();
        }
      }
  ...
  $rootScope.$on(events.FRAME_CONNECTED, () => {
    ...
    getDocumentUriForGroupSearch().then(uri => {
      if (documentUri !== uri) {
        documentUri = uri;
        load();
      }
